...
use WPSignal\WPS;
use const WPSignal\Extensions\WooCommerce\ACTIVITY_CHANNEL;
use const WPSignal\Extensions\WooCommerce\ORDERS_CHANNEL;
use const WPSignal\Extensions\WooCommerce\STOCK_CHANNEL;
...
final class TriggersTest extends ExtensionTestCase {
	...
	public function test_the_extension_is_registered_and_reserves_the_orders_namespace(): void {
		...
		$this->assertArrayHasKey( 'wordsocket-woocommerce', $registered );
		$this->assertSame( array( 'woocommerce/woocommerce.php' => 'WooCommerce' ), $registered['wordsocket-woocommerce']['requires'] );
	...
	public function test_a_stock_change_publishes_publicly_and_a_product_save_does_not_publish_post_updated(): void {
		...
		$this->assertStringContainsString( '4 in stock', wp_strip_all_tags( $data['availability'] ) );
	...
	public function test_adding_to_the_basket_publishes_anonymous_activity_once_per_throttle_window(): void {
		...
		delete_transient( 'wswoo_act_' . $product->get_id() );
		WC()->cart->empty_cart();
		...
		$this->assertSame( array( 'product_id', 'variation_id', 'name', 'permalink', 'quantity', 'actor', 'in_carts' ), array_keys( $data ), 'nothing about who' );
		...
		delete_transient( 'wswoo_act_' . $product->get_id() );
		add_filter( 'wordsocket_woocommerce_activity_enabled', '__return_false' );
		try {
			WC()->cart->add_to_cart( $product->get_id(), 1 );
		} finally {
			remove_filter( 'wordsocket_woocommerce_activity_enabled', '__return_false' );
		}
		$this->assertCount( 2, $this->events_on( ACTIVITY_CHANNEL, 'woo.cart.added' ) );
		WC()->cart->empty_cart();
	}

	public function test_visitor_tokens_carry_the_activity_channel(): void {
		$channels = apply_filters( 'wpsignal_token_channels', array( 'site:s:events' ), 0, 's' );
		$this->assertContains( 'site:s:' . ACTIVITY_CHANNEL, $channels );
	}

	public function test_in_carts_counts_shoppers_and_removals_publish_the_new_count(): void {
		$product = $this->make_product( 10 );
		$id      = $product->get_id();
		delete_transient( 'wswoo_carts_' . $id );
		delete_transient( 'wswoo_act_' . $id );
		WC()->cart->empty_cart();
		$this->published = array();

		// Two other shoppers already have it (tracked by their session hashes).
		\WPSignal\Extensions\WooCommerce\track_cart_add( $id, 'shopper-a' );
		\WPSignal\Extensions\WooCommerce\track_cart_add( $id, 'shopper-b' );
		$this->assertSame( 2, \WPSignal\Extensions\WooCommerce\count_in_carts( $id ) );

		// This session adds it: the event carries the count including us.
		$key = WC()->cart->add_to_cart( $id, 1 );
		$added = $this->events_on( \WPSignal\Extensions\WooCommerce\ACTIVITY_CHANNEL, 'woo.cart.added' );
		...
		$removed = $this->events_on( \WPSignal\Extensions\WooCommerce\ACTIVITY_CHANNEL, 'woo.cart.removed' );
		$this->assertCount( 1, $removed );
		$this->assertSame( array( 'product_id' => $id, 'variation_id' => 0, 'in_carts' => 2 ), $removed[0]['data'] );

		// Abandoned carts age out after the session lifetime.
		set_transient( 'wswoo_carts_' . $id, array( 'shopper-a' => time() - 3 * DAY_IN_SECONDS, 'shopper-b' => time() ), DAY_IN_SECONDS );
		$this->assertSame( 1, \WPSignal\Extensions\WooCommerce\count_in_carts( $id ) );

		// Emptying the cart (what checkout does) untracks every line.
		delete_transient( 'wswoo_act_' . $id );
		WC()->cart->add_to_cart( $id, 1 );
		$this->published = array();
		WC()->cart->empty_cart();
		$emptied = $this->events_on( \WPSignal\Extensions\WooCommerce\ACTIVITY_CHANNEL, 'woo.cart.removed' );
		$this->assertCount( 1, $emptied );
		$this->assertSame( 1, $emptied[0]['data']['in_carts'], 'only shopper-b remains' );
		delete_transient( 'wswoo_carts_' . $id );
	}
}
